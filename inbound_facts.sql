...
with mas_visits as( -- mt.mas.MAS_INBOUND_INC_VW
select 
...
group by
	...
	VISIT_PURPOSE_EN
),border as ( -- MT.tdp.BOARDER_INBOUND_VW
select
...
select
    ...
    m.country,
    m.purpose,
    m.mas_monthly_visitors,
    m.mas_monthly_nights,
    m.mas_avg_daily_visits,
    m.purpose_weight,
    ...
    --m.mas_monthly_visitors * b.border_daily_ratio * m.purpose_weight as mas_daily_visits
    m.mas_monthly_visitors * b.border_daily_ratio as mas_daily_visits
...
select 
	...
	ORIGIN_COUNTRY_NAME_EN as country,
	VISIT_PURPOSE_EN as purpose,
	sum(TRIPS) as mas_monthly_visits,
	sum(NIGHTS) as mas_monthly_nights,
	sum(SPEND_SAR) as mas_monthly_spend,
	sum(TRIPS)/DAY(EOMONTH(DATEFROMPARTS(YEAR, MONTH_NUM, 1))) as mas_avg_daily_visits,
	-- percentage weight of this purpose within the same year-month-country
    sum(TRIPS) * 1.0 / sum(sum(TRIPS)) OVER (partition by YEAR, MONTH_NUM, ORIGIN_COUNTRY_NAME_EN) as purpose_weight
...
--    m.purpose_weight,
    ...
    -- mas daily visits distributed by daily border weight + purpose weight
    --m.mas_monthly_visits * b.border_daily_ratio * m.purpose_weight as mas_daily_visits
    m.mas_monthly_visits * b.border_daily_ratio as mas_daily_visits,
    m.mas_monthly_nights * b.border_daily_ratio as mas_daily_nights,
    m.mas_monthly_spend * b.border_daily_ratio as mas_daily_spend
